...
<hr style={{color: 'hsl(220, 13%, 13%)', fontWeight: 100}}/>
     <ul> 
        {cart.map(({count, image}, index) => (
            <div key={index}>
               <img className='img-cart'  src={image} alt="" /> <span className='desc-cart'>Fall Limited Edition Sneakers <br/>${price}*{count}  <strong> ${total}</strong> <img onClick={() => onDeleteProduct(index)} src={iconDelete} alt="" /> </span> 
            </div>
        ))}
     </ul>
